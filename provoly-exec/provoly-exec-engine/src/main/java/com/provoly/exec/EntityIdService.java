...
import com.provoly.exec.model.EntityId;
...
public class EntityIdService {
    ...
    @Transactional
    public <T extends EntityId> T getById(UUID id, Class<T> entityClass) {
    ...
    public <T extends EntityId> void checkEntityExists(UUID id, Class<T> entityClass) {
        if (em.find(entityClass, id) == null) {
            throw new BusinessException(ErrorCode.BAD_REQUEST, "Entity %s with id %s not exists".formatted(entityClass, id));
        }
    }
